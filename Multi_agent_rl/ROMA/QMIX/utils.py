...
def trans_to_tensor(episode_batch, device):
    ...
    for i in range(len(s_ep)):
        ...
        obs_ep[i] += [[[0.] * len(obs_ep[0][0][0])] * len(obs_ep[0][0])] * (max_ep_len - len(obs_ep[i]))
        obs_next_ep[i] += [[[0.] * len(obs_next_ep[0][0][0])] * len(obs_next_ep[0][0])] * (max_ep_len - len(obs_next_ep[i]))
        a_pre_ep[i] += [[0.] * len(a_pre_ep[0][0])] * (max_ep_len - len(a_pre_ep[i]))
        a_pre_onehot_ep[i] += [[[0.] * len(a_pre_onehot_ep[0][0][0])] * len(a_pre_onehot_ep[0][0])] * (max_ep_len - len(a_pre_onehot_ep[i]))
...
def valid_filter(q_vals, action_mask):
    mask = torch.ones_like(action_mask) * -float("inf") * (1 - action_mask)
    q_vals = q_vals + mask
    return q_vals
