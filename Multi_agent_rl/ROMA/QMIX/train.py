...
def QMIXtrain(replay_buffer, model, target_model, gamma, lr = 1e-3, batch_size = 64):
    ...
    s_ep, a_ep, a_onehot_ep, r_ep, s_next_ep, done_ep, obs_ep, obs_next_ep, a_pre_ep, a_pre_onehot_ep, action_mask_ep, loss_mask_ep = trans_to_tensor(episode_batch, model.device)
    hidden_s = torch.zeros(1, 1, 32).to(model.device)
    q_val_ls = []
    q_target_ls = []
    for i in range(model.num_agent):
        ...
        q_idx, _ = model.agent_model[i](agent_inputs, hidden_state, max_step = 100)
        q_val = torch.gather(q_idx, -1, a_ep[:, :, i])
        q_val_ls.append(q_val)
        
        agent_next_inputs = torch.cat([obs_next_ep[:, :, i, :], a_onehot_ep[:, :, i, :]], -1)
        q_target, _ = target_model.agent_model[i](agent_next_inputs, hidden_s)
        q_target = valid_filter(q_target, action_mask_ep[:, :, i, :])
        q_target = r_ep + gamma * (torch.max(q_target, -1)[0]).unsqueeze(-1) * done_ep
